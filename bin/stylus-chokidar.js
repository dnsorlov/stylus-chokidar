...
var fs = require('fs')
  , stylus = require('stylus/lib/stylus')
...
while (args.length) {
  ...
  switch (arg) {
    ...
    case '--deps':
      ...
      break;
    default:
      files = recursiveReaddirSync(arg)// handle multiple args
  }
}
...
function recursiveReaddirSync(path) {
  ...
  files.forEach(function (file) {
    ...
    if(stats.isDirectory()) {
      list = list.concat(recursiveReaddirSync(join(path, file)));
    } else if(file.match(/\.styl$/)) {
